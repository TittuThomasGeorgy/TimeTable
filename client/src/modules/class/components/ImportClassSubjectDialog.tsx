...
const ImportClassSubjectDialog = (props: Props) => {
    ...
    const handleSubmit = () => {
        ...
        importSubjects(importFrom, {
            onSuccess: (res) => {
                handleClose();
                const alreadyExists = res.data.alreadyExists
                if (alreadyExists.length > 0) {
                    alreadyExists.map(sub => {
                        const subject = classSubjects?.find(clsSub => clsSub._id === sub);
                        enqueueSnackbar({
                            variant: 'error',
                            message: `${(subject?.subject as ISubject).name} already exist`
                        })}
                    )
                }
                const successfulImports = res.data.successfulImports
                if (successfulImports.length > 0) 
                        enqueueSnackbar({
                            variant: 'success',
                            message: `${successfulImports.length} Subjects imported`
                        })}
